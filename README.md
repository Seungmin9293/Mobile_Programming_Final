...
Android 앱은 다음 API 엔드포인트를 통해 백엔드 서버와 통신합니다.

-   **Base URL**: `http://<your-server-address>:8080`

### Exam Schedules API (`/api/schedules`)

| Method | Endpoint | 설명 |
| --- | --- | --- |
| `GET` | `/` | 모든 시험 일정을 조회합니다. |
| `POST` | `/` | 새로운 시험 일정을 생성합니다. |
| `PUT` | `/{id}` | 특정 ID의 시험 일정을 수정합니다. |
| `DELETE` | `/{id}` | 특정 ID의 시험 일정을 삭제합니다. |

### Study Tasks API (`/api/tasks`)

| Method | Endpoint | 설명 |
| --- | --- | --- |
| `GET` | `/` | 모든 학습 과제를 조회합니다. |
| `POST` | `/` | 새로운 학습 과제를 생성합니다. |
| `PUT` | `/{id}` | 특정 ID의 학습 과제를 수정합니다. |
| `DELETE` | `/{id}` | 특정 ID의 학습 과제를 삭제합니다. |
